...
if (-not $Force) {
    if ($PSCmdlet.ShouldProcess("Target $Target", "$Action (DANGEROUS)")) {
        if (-not (Request-ToolkitConfirmation -Verb $Action -Command $Cmd -Config $Config -Arguments $Arguments -Dangerous)) { return }
    } else {
        return
    }
}
