...
switch ($Sub) {
    'ls' {
        ...
        $Cmd = "$BaseCmd images $AllFlag $Filter --format '{{.Repository}}|{{.Tag}}|{{.ID}}|{{.CreatedSince}}|{{.Size}}'"
    ...
    'pull' {
        if (-not $Target) { Write-Host "$($C.Crit)[ERROR] Usage: images pull <image>[:tag]$($C.Reset)" ; return }
        & $BaseCmd pull $Target
    }
    'push' {
        if (-not $Target) { Write-Host "$($C.Crit)[ERROR] Usage: images push <image>[:tag]$($C.Reset)" ; return }
        if (-not (Request-ToolkitConfirmation -Verb "push image" -Command "docker push $Target" -Config $Config -Arguments $Arguments)) { return }
        & $BaseCmd push $Target
    }
    'build' {
        if (-not $Target) { Write-Host "$($C.Crit)[ERROR] Usage: images build <path> [-t name:tag] [--no-cache]$($C.Reset)" ; return }
    ...
    'tag' {
        if (-not $Target -or -not $Tag) { Write-Host "$($C.Crit)[ERROR] Usage: images tag <src> <dst>[:tag]$($C.Reset)" ; return }
        & $BaseCmd tag $Target $Tag
    }
    'rmi' {
        if (-not $Target) { Write-Host "$($C.Crit)[ERROR] Usage: images rmi <image>[:tag] [-f]$($C.Reset)" ; return }
    ...
    'history' {
        if (-not $Target) { Write-Host "$($C.Crit)[ERROR] Usage: images history <image>[:tag]$($C.Reset)" ; return }
        & $BaseCmd history $Target
    }
    default { Write-Host "$($C.Crit)[ERROR] Usage: images [ls|pull|push|build|tag|rmi|prune|history] ...$($C.Reset)" }
}
