...
if (-not (Request-ToolkitConfirmation -Verb "port scan ($($Ports.Count) ports)" -Command "Test-NetConnection to $($Ports -join ',') on $Target" -Config $Config -Arguments $Arguments)) { return }
